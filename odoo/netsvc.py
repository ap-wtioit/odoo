...
try:
    ...
    # 64k - 1k buffer for metadata
    MAX_MESSAGE_LENGTH = 65535 - 1024
    ...
    class RemoteLoggingHandler(Rfc5424SysLogHandler):
        ...
        def __init__(self, **kwargs):
            ...
            structured_data.update({'at_wtioit_odoo_rl_1': {
        ...
        def handle(self, record):
            ...
            structured_data.update({'at_wtioit_odoo_rl_2': {
                'db': record.dbname,
                'name': record.name,
                'model': record.name,
            ...
            if isinstance(record.msg, Sized) and len(record.msg) > MAX_MESSAGE_LENGTH:
                # prevent OSError: [Errno 90] Message too long
                record.msg = record.msg[0:MAX_MESSAGE_LENGTH - 3] + "..."
            super(RemoteLoggingHandler, self).handle(record)

except Exception as e:
    _logger.warning('Cannot create class for remote logging, please install rfc5424logging')
